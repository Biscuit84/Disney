...
import { pionPlayer } from "./plateau-canvas-util";
...
var wMax = $("#canvasesdiv").width(); // on prepare le responsive !
var hMax = $("#canvasesdiv").height();
//console.log(" TEST TEST TEST");
//console.log(wMax +" et "+ hMax);

var w = wMax - (wMax % nBcaseW); // pour couper les cases proprement
var h = hMax - (hMax % nBcaseH);
...
document.getElementById("beforeFireworks").style.display = "none";
document.getElementById("afterFireworks").style.display = "none";

var PR = document.getElementById("pluie");
PR.style.display = "none";
var END = document.getElementById("InterfaceFinDePartie");
//$("#interfaceFinDePartie").css('display','none');
//$("#interfaceFinDePartie").hide();
END.style.display = "none";
document.getElementsByClassName("fondfond").style.display = "none";
//$(".fondfond").css('display','none');
var gameover = document.getElementById("textFinPerdu");
gameover.style.display = "none";
var win = document.getElementById("textFinGagne");
...
pionJoueur: pionPlayer;


// création des objets pion
pionJoueur: pionPlayer = new pionPlayer(0, "../img/player.png", 0, 0, 0, 0, 0, 0, false, 0, false);
pionIA1: pionPlayer;//    = new pionPlayer(1,"../img/ennemi.jpg",0,0,0,0,0,0,false,0,false);
pionIA2: pionPlayer;//   = new pionPlayer(2,"../img/donald.png",0,0,0,0,0,0,false,0,false);
pionIA3: pionPlayer;//   = new pionPlayer(3,"../img/dingo.png",0,0,0,0,0,0,false,0,false);
...
function drawPlayer(pion) {
  ...
  if (pion.positionIndexCasePlayerSuivante <= this.listeCases.length - 1 || pion.positionIndexCasePlayerPrecedente >= 0) {
    if (diff < 0) {
      ...
      if (listeCases[pion.positionIndexCasePlayer].positionCaseY < listeCases[pion.positionIndexCasePlayerSuivante].positionCaseY) {      // avancer case par case
        pion.positionYPlayer += velocityH;
        //console.log("on avance en Y")
      }
      else if (listeCases[pion.positionIndexCasePlayer].positionCaseY > listeCases[pion.positionIndexCasePlayerSuivante].positionCaseY) {        //reculer case par case
        pion.positionYPlayer -= velocityH;
        //console.log("on recule en Y")
      }

      if (listeCases[pion.positionIndexCasePlayer].positionCaseX < listeCases[pion.positionIndexCasePlayerSuivante].positionCaseX) {      // avancer case par case
        pion.positionXPlayer += velocityW;
      }
      else if (listeCases[pion.positionIndexCasePlayer].positionCaseX > listeCases[pion.positionIndexCasePlayerSuivante].positionCaseX) {      //reculer case par case
        pion.positionXPlayer -= velocityW;
      }
    }
    else if (diff > 0) {
      pion.playerIsMoving = true;
      if (listeCases[pion.positionIndexCasePlayer].positionCaseY < listeCases[pion.positionIndexCasePlayerPrecedente].positionCaseY) {      // avancer case par case
        pion.positionYPlayer += velocityH;
      }
      else if (listeCases[pion.positionIndexCasePlayer].positionCaseY > listeCases[pion.positionIndexCasePlayerPrecedente].positionCaseY) {        //reculer case par case
        pion.positionYPlayer -= velocityH;
      }
      if (listeCases[pion.positionIndexCasePlayer].positionCaseX < listeCases[pion.positionIndexCasePlayerPrecedente].positionCaseX) {      // avancer case par case
        pion.positionXPlayer += velocityW;
      }
      else if (listeCases[pion.positionIndexCasePlayer].positionCaseX > listeCases[pion.positionIndexCasePlayerPrecedente].positionCaseX) {      //reculer case par case
        pion.positionXPlayer -= velocityW;
      }
    }
    else if (diff == 0) {
